...
def load_and_preprocess_data(num_clients: int = 3):
    ...
    if os.path.exists(filename):
        ...
        try:
            ...
            # Save locally for next time, preserving '?' for NaNs to match original format
            df.to_csv(filename, header=False, index=False, na_rep="?")
...
def main():
    ...
    # Create a dummy model to get initial parameters (Required for FedAdam)
    net = HeartDiseaseNet()
    ...
    strategy = fl.server.strategy.FedAdam(
        ...
        initial_parameters=initial_parameters, # Required for FedAdam
        eta=0.2, # Server-side learning rate
        eta_l=1e-2, # Client-side learning rate
    )
