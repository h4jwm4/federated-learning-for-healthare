    ...
    for _ in range(epochs):
        for images, labels in trainloader:
            ...
            if mu > 0.0 and global_weights_snapshot is not None:
                prox_term = 0.0
                # Iterate through pairs of local model parameters and the snapshot of the global model
                for local_param, global_param in zip(net.parameters(), global_weights_snapshot):
                    # Proximal term: ||local_param - global_param||^2
                    prox_term += torch.sum(torch.square(local_param - global_param))
                
                # Full FedProx loss: L_k + (mu/2) * ||theta_k - theta_t||^2
...
def test(net, testloader):
    ...
    if total == 0: return 0.0, 0.0
    # Note: We return raw loss here, without the proximal term included
...
class HeartDiseaseClient(fl.client.NumPyClient):
    def __init__(self, cid, location, train_set, local_test_set):
        ...
        # New: Store a snapshot of the global weights received from the server
    ...
    def set_parameters(self, parameters):
        # Store a snapshot of the global weights received for FedProx calculation
    ...
    def fit(self, parameters, config):
        ...
        mu = config.get("mu", 0.0) # Retrieve mu (proximal term coefficient) from server config
        
        # Pass the global weights snapshot and mu to the train function
        loss, accuracy = train(self.net, self.trainloader, epochs=5, 
                               global_weights_snapshot=self.global_weights_snapshot, 
                               mu=mu)
        
        # Log metric with Location info (Note: loss logged here includes the proximal term)
    ...
    def evaluate(self, parameters, config):
        ...
        # Evaluate uses the standard test function (no proximal term in evaluation loss)
...
# FedProx Hyperparameter: Mu coefficient. A non-zero mu enables the proximal term.
FEDPROX_MU = 0.01 
...
def get_on_fit_config_fn():
    """Returns a function that passes mu to the client in the fit config."""
...
def main():
    ...
    # MODIFICATION 1: Set number of clients to 4 (1 client per 4 locations)
    NUM_CLIENTS = 4 
    
    # MODIFICATION 2: Call load_partitioned_data with 1 client per location
    ...
    # Strategy: Use FedAvg for aggregation, but configure FedProx through client config
    # MODIFICATION 3: Update min_fit_clients, min_evaluate_clients, min_available_clients to 4
    strategy = fl.server.strategy.FedAvg(
        ...
        on_fit_config_fn=get_on_fit_config_fn(), # Passes mu=0.01
        on_evaluate_config_fn=get_on_evaluate_config_fn()
    )

    print(f"\nStarting FedProx Simulation (mu={FEDPROX_MU}) with {NUM_CLIENTS} clients across 4 locations (1 client per location).")
    print("This setup maximizes the data size per client, representing 4 large hospital partners.\n")
